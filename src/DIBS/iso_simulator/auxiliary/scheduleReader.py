...
def getSchedule(hk_geb, uk_geb):
    ...
    zuweisungen = pd.read_csv(os.path.join(paths.AUX_DIR, 'occupancy_schedules/occupancy_schedules_zuweisungen.csv'), sep = ';', encoding = 'latin')
    ...
    if hk_geb in zuweisungen['hk_geb'].values:
        ...
        if uk_geb in zuweisungen['uk_geb'].values:
            ...
            df_schedule = pd.read_csv(os.path.join(paths.AUX_DIR, 'occupancy_schedules/')+schedule_name+'.csv', sep = ',')
