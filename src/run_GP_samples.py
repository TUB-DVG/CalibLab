...
def sample_gp(scr_gebaeude_id, num_bc_param, num_samples_sa, num_samples_gp, climate_file, start_time_cal, end_time_cal, output_resolution, training_ratio):
    ...
    # Fetch the intervals for the specific variables
    intervals = inputs.create_intervals(scr_gebaeude_id)

    problem = {'num_vars': num_bc_param,
    ...
    df.to_excel(os.path.join(paths.DATA_DIR, f'GP/{scr_gebaeude_id}/{output_resolution}/{num_bc_param}_bc_param/Indata_GP_{training_ratio}_{num_samples_gp}_samples.xlsx'))
    file_gp_samples = open(os.path.join(paths.DATA_DIR, f'GP/{scr_gebaeude_id}/{output_resolution}/{num_bc_param}_bc_param/Indata_GP_{training_ratio}_{num_samples_gp}_samples.pkl'), "wb")
