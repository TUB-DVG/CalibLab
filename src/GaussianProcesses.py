...
def perform_gp_convergence(scr_gebaeude_id, climate_file, output_resolution, calib_type, start_time_cal, end_time_cal, min_gp_samples, max_gp_samples, num_bc_param, num_samples_sa, step_gp, rmse_threshold, gp_test_size, training_ratio):
    ...
    (2, 'RBF + ExpSine', C(1.0, (1e-3, 1e3)) * RBF(length_scale=1.0, length_scale_bounds=(1e-5, 1e5)) + C(1.0, (1e-3, 1e3)) * ExpSineSquared(length_scale=1.0, periodicity=1.0, length_scale_bounds=(1e-5, 1e5), periodicity_bounds=(1e-5, 1e5))),
    (3, 'RBF', C(1.0, (1e-3, 1e3)) * RBF(length_scale=1.0, length_scale_bounds=(1e-5, 1e5)))]
    ...
    results_df.to_excel(os.path.join(dir_path, f'GP_Conv_all_results_{scr_gebaeude_id}_{output_resolution}_{num_bc_param}_{training_ratio}.xlsx'), index=False)
    results_df.to_csv(os.path.join(dir_path, f'GP_Conv_all_results_{scr_gebaeude_id}_{output_resolution}_{num_bc_param}_{training_ratio}.csv'), index=False)
    
    best_kernel_path = os.path.join(dir_path, f'Best_kernel_{scr_gebaeude_id}_{output_resolution}_{training_ratio}.pkl')
    with open(best_kernel_path, 'wb') as f:
        pickle.dump(best_result['GP_Object'], f)

    best_samples_df_path = os.path.join(dir_path, f'Best_samples_df_{scr_gebaeude_id}_{output_resolution}_{training_ratio}.pkl')
    with open(best_samples_df_path, 'wb') as f:
        pickle.dump(best_result['Samples_DF'], f)

    filename = os.path.join(dir_path, f'GP_Conv{output_resolution}_{best_training_ratio}.txt')
    ...
    print(f"Computational time required for Meta-model generation is {(end_time-start_time):.2f} seconds ({(end_time-start_time)/60:.2f} minutes)")
